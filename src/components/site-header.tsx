      ...
      <div className="mx-auto flex h-[74px] max-w-7xl items-center justify-between px-5 md:px-8">
        ...
        <nav className="hidden items-center gap-8 text-sm font-medium text-black/65 md:flex">
          <div className="mega-trigger group relative py-7">
            <Link className="nav-link transition hover:text-black" href="/portfolio">Portfolio</Link>
          ...
          {navItems.slice(0, 4).filter((item) => item.href !== "/portfolio").map((item) => <Link className="nav-link transition hover:text-black" href={item.href} key={item.href}>{item.label}</Link>)}
        </nav>
