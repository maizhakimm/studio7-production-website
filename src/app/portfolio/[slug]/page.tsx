...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="bg-white text-[#141414]"><SiteHeader />
    <section className="px-5 pb-10 pt-28 md:px-8 md:pb-16 md:pt-36"><div className="mx-auto max-w-7xl text-center"><Link className="text-link inline-flex items-center gap-2 text-sm font-semibold" href="/portfolio"><ArrowLeft size={16} /> All projects</Link><p className="eyebrow mt-10">{project.category}</p><h1 className="display-serif mx-auto mt-4 max-w-4xl text-5xl leading-none md:text-7xl">{project.title}</h1>{project.location && <p className="mt-5 inline-flex items-center gap-2 text-sm text-black/55"><MapPin size={15} /> {project.location}</p>}</div></section>
